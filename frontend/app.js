...
function switchTab(tab, btn) {
    ...
    // Parol tiklash formlarini yashirish
    ['forgotForm','resetCodeForm','newPasswordForm','verifyForm'].forEach(f=>{
        el(f)?.classList.add('hidden');
    });
}
...
function showForgotForm() {
    ['loginForm','registerForm','verifyForm'].forEach(f=>el(f)?.classList.add('hidden'));
    el('forgotForm').classList.remove('hidden');
    el('forgotEmail').focus();
}

function backToLogin() {
    ['forgotForm','resetCodeForm','newPasswordForm'].forEach(f=>el(f)?.classList.add('hidden'));
    el('loginForm').classList.remove('hidden');
}

// 1-bosqich: Email kiritadi → Kod yuboriladi
async function sendForgotCode() {
    const email = v('forgotEmail');
    if(!email || !email.includes('@')) { showErr('forgotError', "Email manzil noto'g'ri"); return; }
    try {
        loading(true);
        await forgotPassword({ email });
        _resetEmail = email;
        el('forgotForm').classList.add('hidden');
        el('resetCodeForm').classList.remove('hidden');
        el('resetEmailText').textContent = email;
        el('resetCode').value = '';
        el('resetCode').focus();
        toast('Kod yuborildi! Emailingizni tekshiring 📧', 'success');
    } catch(e) { showErr('forgotError', 'Xatolik: ' + e.message); }
    finally { loading(false); }
}

// 2-bosqich: Kodni tasdiqlash
async function verifyForgotCode() {
    const code = v('resetCode');
    if(code.length !== 6) { showErr('resetCodeError', '6 raqamli kod kiriting'); return; }
    try {
        loading(true);
        await verifyResetCode({ email: _resetEmail, code });
        _resetCode = code;
        el('resetCodeForm').classList.add('hidden');
        el('newPasswordForm').classList.remove('hidden');
        el('newPassword').focus();
        toast('Kod tasdiqlandi ✅', 'success');
    } catch(e) { showErr('resetCodeError', 'Xatolik: ' + e.message); }
    finally { loading(false); }
}
...
async function resendForgotCode() {
    try {
        loading(true);
        await forgotPassword({ email: _resetEmail });
        el('resetCode').value = '';
        toast('Kod qayta yuborildi 📧', 'success');
    } catch(e) { toast('Xatolik: ' + e.message, 'error'); }
    finally { loading(false); }
...
async function doResetPassword() {
    const newPass = v('newPassword');
    const confirm = v('confirmPassword');
    if(newPass.length < 6) { showErr('newPasswordError', 'Parol kamida 6 ta belgi bolishi kerak'); return; }
        if(newPass !== confirm) { showErr('newPasswordError', 'Parollar mos kelmadi'); return; }
        try {
            loading(true);
            await resetPassword({ email: _resetEmail, code: _resetCode, newPassword: newPass });
            // Muvaffaqiyatli — login sahifaga qaytish
            el('newPasswordForm').classList.add('hidden');
            el('loginForm').classList.remove('hidden');
            el('loginEmail').value = _resetEmail;
            _resetEmail = ''; _resetCode = '';
            toast('Parol muvaffaqiyatli ozgartirildi! Endi kiring 🎉', 'success');
        } catch(e) { showErr('newPasswordError', 'Xatolik: ' + e.message); }
        finally { loading(false); }
    }
...
// ===== STUDENT =====
    ...
    function showSection(name){
        ...
        ['subjects','chapters','topics','topicLearn','test','result','myResults','profile','aiAdvice','school','forgotForm','resetCodeForm','newPasswordForm'].forEach(s=>{
